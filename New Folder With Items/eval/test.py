...
from config import parse_test_file
...
parser.add_argument('--pretrain_packnet', default="",  type=str, help='Checkpoint (.ckpt)')
parser.add_argument('--useBoth', default=False, type=bool)
...
def main():
    ...
    model_wrapper = None
    # load pretained packnet
    if os.path.isfile(args.pretrain_packnet):
        # Parse arguments
        config, state_dict = parse_test_file(args.pretrain_packnet)
       ...
       # model_wrapper = nn.DataParallel(model_wrapper)
        ...
        print('loaded packnet!') 
    ### lassifier model ###
    if args.model == 'lc':
        model = LC(sample_size=args.img_dim, 
                   num_seq=args.num_seq, 
                   seq_len=args.seq_len, 
                   useBoth=args.useBoth,
                   network=args.net,
                   num_class=args.num_class,
                   dropout=args.dropout,
                   packnet=model_wrapper)
        print("successful!!!!")
    else:
    ...
    ### restart training ###
    if args.test:
        print(os.path.isfile(args.test))
...
def train(data_loader, model, optimizer, epoch):
    ...
    for idx, (input_seq, target) in enumerate(data_loader):
        ...
        print("training...epoc: ", epoch)
        output, _ = model(input_seq)
        print("training...out:  ", output.shape)
        # visualize
