...
class LC(nn.Module):
    def __init__(self, sample_size, num_seq, seq_len, 
                 network='resnet18', dropout=0.5, num_class=101, packnet_depth=None):
        ...
        self.packnet_depth = packnet_depth
    ...
    def forward(self, block):
        ...
        # if self.packnet_depth != None:
                ## set the block in the right shape
                # input_block = block.view(B*N, block = block.view(B*N, C, SL, H, W)SL, H, W, C)
                # depth_embedding = self.packnet_depth(input_block)['depth_embedding'][0]
                ## figure out how to get the out to match feature's shape

            ## concat latent spaces
            # feature = feature + depth_embedding
